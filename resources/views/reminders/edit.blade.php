                    ...
                    <form action="{{ route('reminders.update', $reminder->reminder_id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('appointment_id') is-invalid @enderror" 
                                ...
                                @foreach($appointments as $appointment)
                                    <option value="{{ $appointment->appointment_id }}" {{ old('appointment_id', $reminder->appointment_id) == $appointment->appointment_id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="datetime-local" class="form-control @error('reminder_datetime') is-invalid @enderror" 
                                   id="reminder_datetime" name="reminder_datetime" value="{{ old('reminder_datetime', $reminder->reminder_datetime->format('Y-m-d\TH:i')) }}" required>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('reminder_type') is-invalid @enderror" 
                                ...
                                <option value="notification" {{ old('reminder_type', $reminder->reminder_type) == 'notification' ? 'selected' : '' }}>Notification</option>
                            </select>
                            @error('reminder_type')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('status') is-invalid @enderror" 
                                ...
                                <option value="failed" {{ old('status', $reminder->status) == 'failed' ? 'selected' : '' }}>Failed</option>
                            </select>
                            @error('status')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
